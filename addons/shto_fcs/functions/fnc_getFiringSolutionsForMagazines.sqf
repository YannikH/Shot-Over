...

{
	...
	{
		private _rangeTable = [_magazineClass, -5, 66, _x] call shot_over_shto_fcs_fnc_getRangeTable;
		...
		if (_minRange <= _range && _maxRange >= _range) then {
			...
			private _solution = ([_rangeTable, _range, _elevDiff] call shot_over_shto_fcs_fnc_getFiringSolutionForTable);
