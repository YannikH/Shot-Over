...
if (local _howitzer) then {
	[ace_player, _howitzer] call shot_over_shto_m101_fnc_attemptLoad;
} else {
	if (isNull (gunner _howitzer)) then {
		[ace_player, _howitzer] remoteExec ["shot_over_shto_m101_fnc_attemptLoad", _howitzer];
	} else {
		[ace_player, _howitzer] remoteExec ["shot_over_shto_m101_fnc_attemptLoad", gunner _howitzer];
	};
};