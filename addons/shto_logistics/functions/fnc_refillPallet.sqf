...
if (_availableSupply < 0) then {
  [_pallet] call shot_over_shto_logistics_fnc_initPallet; //if a number of crates available hasn't been specified (e.g regular ammo truck) then just refil the pallet
} else {
  [_pallet, _removedCount] call shot_over_shto_logistics_fnc_initPallet;
  if (_supplyClass isKindOF "shto_vn_m101_he_container_x504") then {
    [_supply,_removedCount] call shot_over_shto_logistics_fnc_removeCrates;
  } else {
